\subsection{Problem setup}
We consider temporal pairs of sensor observations $(I_t, I_{t+1})$ together with auxiliary odometry readings. Our goal is to estimate a discretized forward motion state $s\in\{1,\dots,K\}$ robustly when one or more sensor streams are corrupted.

\subsection{Perception (System 1)}
The perception module $\mathcal{P}_\theta$ is a convolutional neural network that maps a stacked 6-channel temporal tensor $X_t = [I_t, I_{t+1}]$ to a categorical probability vector over speed buckets: $p_{cam}=\mathcal{P}_\theta(X_t)\in\Delta^{K-1}$.

\subsection{Differentiable symbolic voting (System 2)}
We compute a joint voting tensor via an Einstein-summation outer product between independent modality distributions (camera, odometer, speed prior). Let $\mathbf{p}_{\text{cam}} \in \Delta^{K-1}$, $\mathbf{p}_{\text{odo}} \in \Delta^{K-1}$, and $\mathbf{p}_{\text{speed}} \in \Delta^{K-1}$ denote the categorical probability distributions from each sensor modality. The joint tensor is defined as:
\begin{equation}
\mathbf{M}_{i,j,k} = \mathbf{p}_{\text{cam},i} \cdot \mathbf{p}_{\text{odo},j} \cdot \mathbf{p}_{\text{speed},k}, \quad i,j,k \in \{1, \ldots, K\}
\label{eq:tensor}
\end{equation}
The isolator evaluates deterministic logical rules over indices of $\mathbf{M}$ (for example, consistency rules that select compatible triples) and reduces $\mathbf{M}$ to an isolated posterior over states. In practice, the reduction is implemented as a weighted sum and a small, fixed set of logical masks $\Phi$ that enforce kinematic feasibility.

\subsection{Architecture schematic}
Figure~\ref{fig:arch} depicts the high-level architecture: a temporal image pair enters the perception CNN (System 1) producing a discrete probability vector; auxiliary odometry and a speed prior produce additional categorical distributions; an Einstein-summation outer product forms a joint tensor (Eq.~\ref{eq:tensor}); deterministic masks $\Phi$ prune inconsistent combinations and produce the final posterior over motion states.

\begin{figure*}[!t]
    \centering
    \resizebox{0.96\linewidth}{!}{%
    \begin{tikzpicture}[
        font=\small,
        >=Latex,
        node distance=0.6cm and 1.2cm,
        every node/.style={align=center},
        base/.style={draw, thick, rounded corners=4pt, minimum height=1.1cm, text width=2.8cm, inner sep=5pt, fill=white},
        sensor/.style={base, fill=gray!8, draw=gray!55!black},
        neural/.style={base, fill=blue!6, draw=blue!55!black, text width=3.2cm},
        symbolic/.style={base, fill=orange!6, draw=orange!65!black, text width=3.4cm},
        output/.style={base, fill=green!6, draw=green!50!black, text width=2.4cm},
        arrow/.style={->, thick, draw=gray!70!black}
    ]
        % Stage 1: Left Input Column
        \node[sensor] (frames) {6-channel temporal stack\\$[I_t, I_{t+1}]$};
        \node[sensor, below=of frames] (speed) {Speed prior\\$\mathbf{p}_{\text{speed}}$};
        \node[sensor, below=of speed] (odo) {Odometer\\$\mathbf{p}_{\text{odo}}$};

        % Stage 2: System 1 Perception
        \node[neural, right=of frames] (cnn) {Perception CNN\\$\mathcal{P}_\theta$\\$\mathbf{p}_{\text{cam}} \in \Delta^{K-1}$};

        % Stage 3: System 2 Deterministic Outer Product
        \node[symbolic, right=1.6cm of cnn.south east, yshift=-0.55cm, anchor=west] (outer) {
            Tensor outer product\\
            $\mathbf{M}_{i,j,k} = \mathbf{p}_{\text{cam},i}\,\mathbf{p}_{\text{odo},j}\,\mathbf{p}_{\text{speed},k}$
        };
        \node[symbolic, right=of outer] (mask) {Symbolic mask ($\Phi$)\\Kinematic rules};
        
        % Stage 4: Safety Action Evaluation
        \node[output, right=of mask] (posterior) {Posterior\\$\mathbf{P}(V=m)$};

        % Decorative Containment Blocks
        \begin{scope}[on background layer]
            \node[draw=blue!40, dashed, rounded corners=6pt, fill=blue!4, fill opacity=0.3, inner sep=12pt, fit=(frames) (cnn), label={[blue!70!black]above:\textbf{System 1: Neural perception}}] (bg_sys1) {};
            \node[draw=orange!40, dashed, rounded corners=6pt, fill=orange!4, fill opacity=0.3, inner sep=12pt, fit=(outer) (mask), label={[orange!70!black]above:\textbf{System 2: Symbolic constraints}}] (bg_sys2) {};
        \end{scope}

        % Alignment Arrow Matrix Routing
        \draw[arrow] (frames) -- (cnn);
        \draw[arrow] (cnn.east) -- node[above, font=\scriptsize, pos=0.3] {$\mathbf{p}_{\text{cam}}$} ($(outer.west)+(0,0.35)$);
        \draw[arrow] (speed.east) -- node[above, font=\scriptsize, pos=0.5] {$\mathbf{p}_{\text{speed}}$} ($(outer.west)+(0,0.0)$);
        \draw[arrow] (odo.east) -- node[below, font=\scriptsize, pos=0.5] {$\mathbf{p}_{\text{odo}}$} ($(outer.west)+(0,-0.35)$);
        \draw[arrow] (outer) -- node[above, font=\scriptsize] {$\mathbb{R}^{K \times K \times K}$} (mask);
        \draw[arrow] (mask) -- node[above, font=\scriptsize] {$\mathbb{R}^{K}$} (posterior);
    \end{tikzpicture}%
    }
    \caption{The optimized temporal neuro-symbolic execution pipeline. By decoupling the spatial positions of the raw sensor feeds from the neural processing block, data vector arrays route straight into the tensor outer product without overlapping or visual interference. System 1 processes high-dimensional pixel sequences; System 2 enforces deterministic kinematic safety constraints.}
    \label{fig:arch}
\end{figure*}

\subsection{Training objective}
We train $\mathcal{P}_\theta$ with a standard cross-entropy objective against discretized ground-truth speed buckets, while keeping the symbolic masks deterministic and non-trainable. This preserves end-to-end differentiability for the perception network while guaranteeing the invariants encoded by $\Phi$.